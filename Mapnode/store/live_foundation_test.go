...
func TestDependencyEvidenceMigrationIsV7AndPreservesPriorChecksums(t *testing.T) {
	db := openTestDB(t)
	for _, table := range []string{"live_chains", "canonical_cursors", "trust_root_observations", "live_indexer_configs", "indexer_degraded_states", "indexed_gateway_logs", "verification_receipts", "request_resolutions", "evidence_inbox", "evidence_outbox", "evidence_ingress_rejections"} {
		var strict int
	...
	if err := db.sql.QueryRow("SELECT MAX(version),COUNT(*) FROM schema_migrations").Scan(&version, &count); err != nil || version != 8 || count != 8 {
		t.Fatalf("migration history max=%d count=%d err=%v", version, count, err)
	}
	checksums := [][32]byte{sha256.Sum256([]byte(phase3Schema)), sha256.Sum256([]byte(pathProofIntegrityV2)), sha256.Sum256([]byte(liveObservationFoundationV3)), sha256.Sum256([]byte(liveChainDeleteProtectionV4)), sha256.Sum256([]byte(confirmedGatewayIndexerV5)), sha256.Sum256([]byte(dependencyEvidenceMailboxesV6)), sha256.Sum256([]byte(evidenceIngressRejectionsV7))}
	wants := []string{"ab9d49540980805aa7f76dd84a4c99bce4df85ed61f4f9eeb3285c91ec40dc4a", "795e28e399ac9df3d8a39a1dbf309ca964de3025a5f4090758bd98a2eda8841e", "dff57af6e30db4a26d11a45c29ac46a5425f563552b6a5430af05b9f69c3a088", "b2d912d7120e34420424e9f33b8cfa9df3c2c3fb829d2d13c67b63a312634fc5", "1eca6274db154e9fe9ad53c6f28cade7a7d03f9e881db2769667297c1a06c9ea", "07b7822824d68aba311415023a98a1f320c45122f1fe38437d4c0588cd68d49f", "e90eca38009e5425fd37a4bb1e04e580536b10af58706664706f60559cea30f6"}
...
func TestIngressRejectionMigrationIsV7AndUpgradesRealV6WithoutChecksumDrift(t *testing.T) {
	...
	for _, item := range migrations[:6] {
		if _, err := raw.Exec(item.sql); err != nil {
			t.Fatalf("apply historical v%d: %v", item.version, err)
		}
		checksum := sha256.Sum256([]byte(item.sql))
		if _, err := raw.Exec("INSERT INTO schema_migrations(version,name,checksum,applied_at) VALUES(?,?,?,?)", item.version, item.name, checksum[:], time.Now().UTC().UnixNano()); err != nil {
			t.Fatal(err)
		}
	}
	...
	defer db.Close()
	var version, count, strict int
	if err := db.sql.QueryRow("SELECT MAX(version),COUNT(*) FROM schema_migrations").Scan(&version, &count); err != nil || version != 8 || count != 8 {
		t.Fatalf("history max=%d count=%d err=%v", version, count, err)
	}
	if err := db.sql.QueryRow("SELECT strict FROM pragma_table_list WHERE name='evidence_ingress_rejections'").Scan(&strict); err != nil || strict != 1 {
		t.Fatalf("ingress rejection table strict=%d err=%v", strict, err)
	}
	for _, item := range migrations[:6] {
		want := sha256.Sum256([]byte(item.sql))
		var got []byte
		if err := db.sql.QueryRow("SELECT checksum FROM schema_migrations WHERE version=?", item.version).Scan(&got); err != nil || !equalBytes(got, want[:]) {
			t.Fatalf("v%d checksum changed err=%v", item.version, err)
		}
	}
}

func TestDependencyEvidenceMigrationUpgradesRealV4DatabaseThroughV7WithoutChecksumDrift(t *testing.T) {
