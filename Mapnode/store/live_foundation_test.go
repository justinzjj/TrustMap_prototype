...
func TestDependencyEvidenceMigrationIsV7AndPreservesPriorChecksums(t *testing.T) {
	...
	if err := db.sql.QueryRow("SELECT MAX(version),COUNT(*) FROM schema_migrations").Scan(&version, &count); err != nil || version != 7 || count != 7 {
		t.Fatalf("migration history max=%d count=%d err=%v", version, count, err)
	}
	checksums := [][32]byte{sha256.Sum256([]byte(phase3Schema)), sha256.Sum256([]byte(pathProofIntegrityV2)), sha256.Sum256([]byte(liveObservationFoundationV3)), sha256.Sum256([]byte(liveChainDeleteProtectionV4)), sha256.Sum256([]byte(confirmedGatewayIndexerV5)), sha256.Sum256([]byte(dependencyEvidenceMailboxesV6))}
	wants := []string{"ab9d49540980805aa7f76dd84a4c99bce4df85ed61f4f9eeb3285c91ec40dc4a", "795e28e399ac9df3d8a39a1dbf309ca964de3025a5f4090758bd98a2eda8841e", "dff57af6e30db4a26d11a45c29ac46a5425f563552b6a5430af05b9f69c3a088", "b2d912d7120e34420424e9f33b8cfa9df3c2c3fb829d2d13c67b63a312634fc5", "1eca6274db154e9fe9ad53c6f28cade7a7d03f9e881db2769667297c1a06c9ea", "07b7822824d68aba311415023a98a1f320c45122f1fe38437d4c0588cd68d49f"}
